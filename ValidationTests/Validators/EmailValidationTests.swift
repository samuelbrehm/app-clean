...
class EmailValidationTests: XCTestCase {
    func test_validate_should_return_error_if_invalid_email_is_provided() {
        ...
        let errorMessage = sut.validate(data: ["email": "invalid_email@email.com"])
    ...
    func test_validate_should_return_nil_with_valid_email_is_provided() {
        let sut = makeSut(fieldName: "email", fieldLabel: "Email2", emailValidator: EmailValidatorSpy())
        let errorMessage = sut.validate(data: ["email": "valid_email@email.com"])
